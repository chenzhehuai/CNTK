...
void NDLNodeEvaluatorImpl<ElemType>::Evaluate(NDLNode<ElemType>* node, const wstring& baseName, const NDLPass pass)
{
	...
	else if (cnNodeType == OperationNameOf(LatticeFreeMMINodeN))
    {
        if (parameter.size() != 3)
            RuntimeError("%ls should have 3 fixed parameters[labelVectorSequence, outProbVectorSequence, logPrior].", cnNodeType.c_str());
        ...
        if (pass == ndlPassInitial)
        {
            ...
            nodePtr = builder.LatticeFreeMMIN(nullptr, nullptr, nullptr, msra::strfun::utf16(fstFilePath), msra::strfun::utf16(smapFilePath), squashingFactor, alignmentWindow, ceweight, boosted, name);
        }
    }
