...
template <class ElemType>
class LatticeFreeMMINodeN : public ComputationNodeNonLooping /*ComputationNode*/<ElemType>, public NumInputs<3>
{
    typedef ComputationNodeNonLooping<ElemType> Base;
    UsingComputationNodeMembersBoilerplate;
    static const std::wstring TypeName()
    {
        return L"LatticeFreeMMIN";
    }
...
public:
    LatticeFreeMMINodeN(DEVICEID_TYPE deviceId, const wstring& name)
        : Base(deviceId, name), m_squashingFactor(1.0), m_alignmentWindow(0), m_ceweight(0), m_totalFrameNumberOfCurrentMinibatch(0), m_boosted(0)
    {
        InitMatrixes();
    }

    LatticeFreeMMINodeN(DEVICEID_TYPE deviceId, const wstring& name, const wstring& fstFilePath, const wstring& smapFilePath, const ElemType squashingFactor, const int alignmentWindow, const ElemType ceweight, const ElemType boosted)
        : Base(deviceId, name), m_squashingFactor(squashingFactor), m_alignmentWindow(alignmentWindow), m_ceweight(ceweight), m_totalFrameNumberOfCurrentMinibatch(0), m_boosted(boosted)
    {
        InitMatrixes();
        InitializeFromTfstFiles(fstFilePath, smapFilePath);
    ...
    LatticeFreeMMINodeN(const ScriptableObjects::IConfigRecordPtr configp)
        : LatticeFreeMMINodeN(configp->Get(L"deviceId"), L"<placeholder>", configp->Get(L"fstFilePath"), configp->Get(L"smapFilePath"), configp->Get(L"squashingFactor"), configp->Get(L"alignmentWindow"), configp->Get(L"ceweight"), configp->Get(L"boosted"))
    {
        AttachInputsFromConfig(configp, this->GetExpectedNumInputs());
    }
...
#endif
#if DUMPOUTPUT
        Value().Print("LatticeFreeMMINodeN");
#endif
    ...
    virtual void /*ComputationNodeBase::*/ Validate(bool isFinalValidationPass) override
    {
        ...
        if (isFinalValidationPass)
        {
            ...
            //auto c4 = Input(4)->ValueAsMatrix().GetNumCols();
            //if (r0 != r4 || c3 != r3 || c3 != c4)
            //    LogicError("The Matrix dimension in the LatticeFreeMMINodeN operation does not match.");
        }
    }
    ...
    virtual void CopyTo(ComputationNodeBasePtr nodeP, const std::wstring& newName, const CopyNodeFlags flags) const override
    {
        ...
        if (flags & CopyNodeFlags::copyNodeValue)
        {
            auto node = dynamic_pointer_cast<LatticeFreeMMINodeN<ElemType>>(nodeP);
            node->m_squashingFactor = m_squashingFactor;
            node->m_alignmentWindow = m_alignmentWindow;
            node->m_ceweight = m_ceweight;
            node->m_boosted = m_boosted;			
